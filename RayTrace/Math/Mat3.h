...
public:
	...
	Mat3();
	Mat3(const Mat3<T>& inputMatrix);
	Mat3(const std::vector<T>& inputData);
	...
	// Overload == operator.
	bool operator== (const Mat3<T>& rhs);
	bool Compare(const Mat3<T>& matrix1, double tolerance);

	// Overload the assignment operator.
	Mat3<T> operator= (const Mat3<T>& rhs);

	// Overload +, - and * operators (friends).
	template <class U> friend Mat3<U> operator+ (const Mat3<U>& lhs, const Mat3<U>& rhs);
	template <class U> friend Mat3<U> operator+ (const U& lhs, const Mat3<U>& rhs);
	template <class U> friend Mat3<U> operator+ (const Mat3<U>& lhs, const U& rhs);
	...
	// Mat3 * Vec.
	template <class U> friend Vec<U> operator* (const Mat3<U>& lhs, const Vec<U>& rhs);
...
template <class T>
Mat3<T>::Mat3(const Mat3<T>& inputMatrix)
{
	for (int i = 0; i < 9; i++)
...
template <class T>
Mat3<T>::Mat3(const std::vector<T>& inputData)
{
	if (inputData.size() != 9)
		throw std::invalid_argument("Cannot assign std::vector to Mat3 - assignment dimension mismatch.");
...
/* *************************************************************************************************/
template <class T>
T Mat3<T>::GetElement(int row, int col) const
{
...
template <class T>
bool Mat3<T>::SetElement(int row, int col, T elementValue)
{
	int linearIndex = Sub2Ind(row, col);
	if (linearIndex >= 0)
	{
...
template <class T>
int Mat3<T>::GetNumRows() const
{
	return m_nRows;
}

template <class T>
int Mat3<T>::GetNumCols() const
{
	return m_nCols;
}

template <class T>
bool Mat3<T>::Compare(const Mat3<T>& matrix1, double tolerance)
{
...
// scaler + matrix
template <class T>
Mat3<T> operator+ (const T& lhs, const Mat3<T>& rhs)
{
	Mat3<T> result;
	for (int i = 0; i < 9; ++i)
...
// matrix + scaler
template <class T>
Mat3<T> operator+ (const Mat3<T>& lhs, const T& rhs)
{
	Mat3<T> result;
	for (int i = 0; i < 9; ++i)
...
// matrix * Vec
template <class T>
Vec<T> operator* (const Mat3<T>& lhs, const Vec<T>& rhs)
{
...
/* *************************************************************************************************/
template <class T>
bool Mat3<T>::operator== (const Mat3<T>& rhs)
{
...
bool Mat3<T>::Inverse()
{
	...
	// And transpose.
	Mat3<T> adjT = adjugate.Transpose();
	...
	Mat3<T> result = (1.0 / determinant) * adjT;
...
template <class T>
int Mat3<T>::Sub2Ind(int row, int col) const
{
...
template <class T>
T Mat3<T>::cofactorDeterminant(T e1, T e2, T e3, T e4)
{
	return (e1 * e4) - (e2 * e3);
}

template <class T>
bool Mat3<T>::CloseEnough(T f1, T f2)
{
