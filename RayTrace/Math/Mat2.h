...
public:
	...
	Mat2();
	Mat2(int nRows, int nCols);
	Mat2(int nRows, int nCols, const T* inputData);
	Mat2(const Mat2<T>& inputMatrix);
	Mat2(int nRows, int nCols, const std::vector<T>& inputData);
	...
	// Overload == operator.
	bool operator== (const Mat2<T>& rhs);
	bool Compare(const Mat2<T>& matrix1, double tolerance);

	// Overload the assignment operator.
	Mat2<T> operator= (const Mat2<T>& rhs);

	// Overload +, - and * operators (friends).
	template <class U> friend Mat2<U> operator+ (const Mat2<U>& lhs, const Mat2<U>& rhs);
	template <class U> friend Mat2<U> operator+ (const U& lhs, const Mat2<U>& rhs);
	template <class U> friend Mat2<U> operator+ (const Mat2<U>& lhs, const U& rhs);
	...
	// Mat2 * Vec (size 2).
	template <class U> friend Vec<U> operator* (const Mat2<U>& lhs, const Vec<U>& rhs);
	...
	bool Separate(Mat2<T>& matrix1, Mat2<T>& matrix2, int colNum);
	bool Join(const Mat2<T>& matrix2);
	Mat2<T> FindSubMatrix(int rowNum, int colNum);
...
template <class T>
Mat2<T>::Mat2(int nRows, int nCols)
{
	m_nRows = nRows;
	m_nCols = nCols;
...
template <class T>
Mat2<T>::Mat2(int nRows, int nCols, const T* inputData)
{
	m_nRows = nRows;
	m_nCols = nCols;
	m_nElements = m_nRows * m_nCols;
...
template <class T>
Mat2<T>::Mat2(const Mat2<T>& inputMatrix)
{
...
template <class T>
Mat2<T>::Mat2(int nRows, int nCols, const std::vector<T>& inputData)
{
	m_nRows = nRows;
	m_nCols = nCols;
	m_nElements = m_nRows * m_nCols;
...
/* *************************************************************************************************/
template <class T>
bool Mat2<T>::Resize(int numRows, int numCols)
{
	m_nRows = numRows;
	m_nCols = numCols;
	m_nElements = (m_nRows * m_nCols);
...
/* *************************************************************************************************/
template <class T>
T Mat2<T>::GetElement(int row, int col) const
{
...
template <class T>
bool Mat2<T>::SetElement(int row, int col, T elementValue)
{
	int linearIndex = Sub2Ind(row, col);
	if (linearIndex >= 0)
	{
...
template <class T>
int Mat2<T>::GetNumRows() const
{
	return m_nRows;
}

template <class T>
int Mat2<T>::GetNumCols() const
{
	return m_nCols;
}

template <class T>
bool Mat2<T>::Compare(const Mat2<T>& matrix1, double tolerance)
{
...
Mat2<T> operator+ (const Mat2<T>& lhs, const Mat2<T>& rhs)
{
	...
	Mat2<T> result(numRows, numCols, tempResult);
	delete[] tempResult;
	return result;
}
...
// scaler + matrix
template <class T>
Mat2<T> operator+ (const T& lhs, const Mat2<T>& rhs)
{
...
// matrix + scaler
template <class T>
Mat2<T> operator+ (const Mat2<T>& lhs, const T& rhs)
{
	int numRows = lhs.m_nRows;
	int numCols = lhs.m_nCols;
	int numElements = numRows * numCols;
...
Mat2<T> operator- (const Mat2<T>& lhs, const Mat2<T>& rhs)
{
	...
	Mat2 result(numRows, numCols, tempResult);
	delete[] tempResult;
	return result;
}
...
// matrix * vector
template <class T>
Vec<T> operator* (const Mat2<T>& lhs, const Vec<T>& rhs)
{
...
Mat2<T> operator* (const Mat2<T>& lhs, const Mat2<T>& rhs)
{
	...
	if (l_numCols == r_numRows)
	{
		...
		Mat2<T> result(l_numRows, r_numCols, tempResult);
		delete[] tempResult;
		return result;
	}
	else
...
/* *************************************************************************************************/
template <class T>
bool Mat2<T>::operator== (const Mat2<T>& rhs)
{
...
(Note that the output is returned into the two Mat2<T> pointers in the input argument list)
/* *************************************************************************************************/
template <class T>
bool Mat2<T>::Separate(Mat2<T>& matrix1, Mat2<T>& matrix2, int colNum)
{
...
/* *************************************************************************************************/
template <class T>
bool Mat2<T>::Join(const Mat2<T>& matrix2)
{
...
T Mat2<T>::Determinant()
{
	...
	else
	{
		...
		for (int j = 0; j < m_nCols; ++j)
		{
			...
			Mat2<T> subMatrix = this->FindSubMatrix(0, j);
...
bool Mat2<T>::Inverse()
{
	...
	Mat2<T> identityMatrix(m_nRows, m_nCols);
...
Mat2<T> Mat2<T>::Transpose() const
{
	...
	Mat2<T> resultMatrix(m_nCols, m_nRows);
...
Mat2<T> Mat2<T>::RowEchelon()
{
	...
	// Form the output matrix.
	Mat2<T> outputMatrix(m_nRows, m_nCols, m_matrixData);
...
int Mat2<T>::Rank()
{
	...
	Mat2<T> matrixCopy = this->RowEchelon();
	...
	if (!matrixCopy.IsRowEchelon())
	{
		...
		std::vector<Mat2<T>> subMatrixVector;
		...
		while ((subMatrixCount < subMatrixVector.size()) && (!completeFlag))
		{
			...
			Mat2<T> currentMatrix = subMatrixVector[subMatrixCount];
			subMatrixCount++;
...
template <class T>
int Mat2<T>::Sub2Ind(int row, int col) const
{
...
template <class T>
void Mat2<T>::SwapRow(int i, int j)
{
...
template <class T>
void Mat2<T>::MultAdd(int i, int j, T multFactor)
{
	for (int k = 0; k < m_nCols; ++k)
...
template <class T>
int Mat2<T>::FindRowWithMaxElement(int colNumber, int startingRow)
{
...
template <class T>
void Mat2<T>::MultRow(int i, T multFactor)
{
	for (int k = 0; k < m_nCols; ++k)
...
template <class T>
void Mat2<T>::PrintMatrix(int precision)
{
	int nRows = this->GetNumRows();
	int nCols = this->GetNumCols();
	for (int row = 0; row < nRows; ++row)
...
template <class T>
bool Mat2<T>::CloseEnough(T f1, T f2)
{
